...
template <std::equality_comparable T> class LLNode {
public:
...
template <std::equality_comparable T> void LLNode<T>::remove_from_list() {
  next->prev = prev;
  prev->next = next;
...
template <std::equality_comparable T> class LList {
public:
...
template <std::equality_comparable T> int LList<T>::length() {
  LLNode<T> *element = this->head;
  int r = 0;
  while (element != nullptr) {
    element = element->next;
...
template <std::equality_comparable T> LList<T>::~LList() {
  if (head == nullptr)
    return;
...
template <std::equality_comparable T> void LList<T>::remove_dups() {
  auto n = this->head;
  while (n != nullptr) {
...
// default impl
template <std::equality_comparable T> void LList<T>::print_nodes() {}

template <> inline void LList<int>::print_nodes() {
  auto x = this->head;
  while (x != nullptr) {
    printf("data = %d\n", x->data);
    x = x->next;
...
template <std::equality_comparable T> bool LList<T>::operator==(LList<T> &rhs) {
...
template <std::equality_comparable T> LLNode<T> LList<T>::nth_last(int n) {
  int i = (length() - n - 1);
  LLNode<T> *ret = this->head;
